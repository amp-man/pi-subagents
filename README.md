...
Blocking calls in the **main conversation** are disabled by default to prevent self-blocking (foreground `Agent` runs and `get_subagent_result({ wait: true })`). Nested subagents can still use `wait: true`. You can enable blocking in `/agents` → Settings (`Allow main conversation blocking`).
### `steer_subagent`
...
Settings                                    ← max concurrency, max turns, grace turns, join mode, main blocking toggle
```
...
- **Settings** — configure max concurrency, default max turns, grace turns, join mode, and whether the main conversation may use blocking calls

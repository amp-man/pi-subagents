...
/** Allow main conversation to use blocking subagent calls. */
let allowBlocking = false;

/** Get whether blocking is allowed for the main conversation. */
export function getAllowBlocking(): boolean { return allowBlocking; }
/** Set whether blocking is allowed for the main conversation. */
export function setAllowBlocking(b: boolean): void { allowBlocking = b; }
...
export async function runAgent(
  ...
  if (extensions !== false) {
    ...
    const activeTools = session.getActiveToolNames().filter((t) => {
      if (!agentConfig?.allowNesting && EXCLUDED_TOOL_NAMES.includes(t)) return false;
